...
@section('title')
    <title>Data Program Studi | Presensi Teknik Elektro</title>
@endsection

@section('header-table')
    <button type="button" name="create_record" id="create_record" class="btn btn-primary float-end">Tambah Data</button>
    <h6 class="m-0 font-weight-bold text-primary">Program Studi Tabel</h6>
@endsection
    ...
    <div class="container">
        <div class="row">
            <div class="col-12 table-responsive">
                <table class="table table-striped table-bordered prodi_datatable">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Name</th>
...
<script type="text/javascript">
    $(document).ready(function() {
        var table = $('.prodi_datatable').DataTable({
            processing: true,
            serverSide: true,
        ...
        $('#sample_form').on('submit', function(event){
            ...
            if($('#action').val() == 'Add')
            {
                action_url = "{{ route('prodi.store') }}";
            }

            if($('#action').val() == 'Edit')
            {
                action_url = "{{ route('prodi.update') }}";
            }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if(data.success)
                    {
                        ...
                        $('#sample_form')[0].reset();
                        $('#prodi_table').DataTable().ajax.reload();
                    }
        ...
        $(document).on('click', '.delete', function(){
            ...
            prodi_id = $(this).attr('id');
            $('#confirmModal').modal('show');
        });

        $('#ok_button').click(function(){
            $.ajax({
                url:"/admin/prodi/destroy/"+prodi_id,
                beforeSend:function(){
